...
class ConnectedComponent:
    ...
    def get_edge_strip(self, side: str, strip_size: int = 5) -> np.ndarray:
        """
        Returns a 2D rectangular strip for the given side of shape:
            (strip_size, width, 3) for top/bottom
            (height, strip_size, 3) for left/right

        Masked-out pixels are kept but set to black (0,0,0) so that
        spatial structure is preserved for matching.
        """
        ...
        # Select strip region
        if side == "top":
            rows = slice(0, strip_size)
            cols = slice(0, W)
        elif side == "bottom":
            rows = slice(H - strip_size, H)
            cols = slice(0, W)
        elif side == "left":
            rows = slice(0, H)
            cols = slice(0, strip_size)
        elif side == "right":
            rows = slice(0, H)
            cols = slice(W - strip_size, W)
        else:
            raise ValueError("Invalid side")

        # Extract regions
        strip = patch[rows, cols].copy()
        strip_mask = mask[rows, cols]

        # Apply mask → keep structure, but zero-out non-piece pixels
        strip[strip_mask == 0] = 0  # black or choose 255 for white background

        return strip  # returns a 2D shape
